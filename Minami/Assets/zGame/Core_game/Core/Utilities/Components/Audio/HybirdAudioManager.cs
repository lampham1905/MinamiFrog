...
namespace Lam.zGame.Core_game.Core.Utilities.Components.Audio
{
    ...
    public class HybirdAudioManager : BaseAudioManager
    {
        ...
        public void PlaySFXById(int pId, bool pLoop = false, float pPitchRandomMultiplier = 1)
        {
            ...
            if (clip != null)
                PlaySFX(clip, clip.limitNumber, pLoop, pPitchRandomMultiplier);
        }
        ...
        {
            if (idMusicJustPlay == pId&& mMusicSource.isPlaying) {
                return;
            }
            int outIndex = -1;
            ...
            if (clip != null)
            {
                PlayMusic(clip.GetClipPlayMusic(), false, pFadeDuration, pVolume);
            }
            idMusicJustPlay = pId;
        }
        ...
        //    if (clip != null)
        //        PlayMusic(clip.GetClipPlaySFX(), pLoop, pFadeDuration, pVolume);
        //}
        #endregion
