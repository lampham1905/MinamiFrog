...
namespace Lam.zGame.Core_game.Core.Utilities.Components.UI
{
    ...
    public class JustButton : Button
    {
        ...
        [SerializeField] protected bool mGreyMatEnabled = true;
        ...
        public Action holdButton;
        float timeHold = 0.5f;
        float countTimeHold = 0.5f;
        ...
        private void Update()
        {
            //IsPressed is a public function in the selectable class which button inherits from.
            if (IsPressed() && holdButton != null)
            {
                countTimeHold -= Time.unscaledDeltaTime;
                if (countTimeHold <= 0)
                {
                    WhilePressed();
                }
        ...
        /// <summary>
        /// invoke an event while the button is pressed
        /// </summary>
