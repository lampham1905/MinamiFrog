...
namespace Lam.zGame.Core_game.Core.Utilities.Common.Extensions_And_Helper
{
    public static class ComponentExtension
    {
        ...
        public static void SetActive(this Component target, bool value)
        {
            ...
            {
                Debug.Debug.LogError(ex.ToString());
            }
        }
        ...
        public static Vector2 SetNativeSize(this UnityEngine.UI.Image pImage, Vector2 pMaxSize)
        {
            ...
            //-----dai-------
            bool isLonHon = false;
            if (pMaxSize.x != 0 && pMaxSize.y != 0)
            {
                float rateX = nativeSizeX / pMaxSize.x;
                float rateY = nativeSizeY / pMaxSize.y;
                if (rateX > 1f || rateY > 1f)
                {
                    isLonHon = true;
                    if (rateX > rateY)
                    {
                        ...
                        nativeSizeY = nativeSizeY / (rateX);
                    }
                    else
                    {
                        //x scale theo y
                        nativeSizeY = pMaxSize.y;
                        nativeSizeX = nativeSizeX / (rateY);
                    }
                }
            }
            //----------------
