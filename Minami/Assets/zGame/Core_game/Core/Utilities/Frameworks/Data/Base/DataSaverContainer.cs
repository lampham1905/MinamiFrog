...
namespace Lam.zGame.Core_game.Core.Utilities.Frameworks.Data.Base
{
    ...
    public class DataSaverContainer
    {
        ...
        public static DataSaver CreateSaver(string pIdString, IEncryption pEncryption, bool storeSaverKey = true)
        {
            if (savers.ContainsKey(pIdString))
            {
            ...
            if(storeSaverKey) StoreSaverKey(pIdString);
        ...
        public static void RemoveSaverKey(string pKey)
        {
            savers.Remove(pKey);
            PlayerPrefs.DeleteKey(pKey);
        }

        public static void ReplaceSaver(string pKey, DataSaver dataSaver)
        {
            savers[pKey] = dataSaver;
            savers[pKey].idString = pKey;
        }
        ...
        public static void ImportData(string pJsonData)
        {
            ...
            if (saverBrands != null)
            {
                foreach (var brand in saverBrands)
                {
                    ...
                    if (USE_BINARY)
                        BinarySaver.SaveBinary(brand.v, brand.k);
