...
public class QuickTouch : QuickBase {
	...
	void Update(){
		...
		if (!is2Finger){
			...
			// start
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_TouchStart && actionTriggering == ActionTriggering.Start){
			...
			// Down
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_TouchDown  && actionTriggering == ActionTriggering.Down){
				if (currentGesture.fingerIndex == fingerIndex || isMultiTouch){
					DoAction( currentGesture);
				}
			}
			...
			// Up
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_TouchUp){
			...
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_TouchStart2Fingers && actionTriggering == ActionTriggering.Start){
				DoAction( currentGesture);
			}
			
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_TouchDown2Fingers && actionTriggering == ActionTriggering.Down){
				DoAction( currentGesture);
			}
			
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_TouchUp2Fingers && actionTriggering == ActionTriggering.Up){
				DoAction( currentGesture);
			}
		}
	}
