...
public class QuickLongTap : QuickBase {
	...
	void Update(){
		...
		if (!is2Finger){
			...
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_TouchStart && fingerIndex == -1 && IsOverMe(currentGesture)){
				fingerIndex = currentGesture.fingerIndex;
			}
			
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_LongTapStart && actionTriggering == ActionTriggering.Start){
			...
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_LongTap  && actionTriggering == ActionTriggering.InProgress){
				if (currentGesture.fingerIndex == fingerIndex || isMultiTouch){
					DoAction( currentGesture);
				}
			}
			
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_LongTapEnd && actionTriggering == ActionTriggering.End){
				if (currentGesture.fingerIndex == fingerIndex || isMultiTouch){
					DoAction( currentGesture);
					fingerIndex =-1;
				}
			...
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_LongTapStart2Fingers && actionTriggering == ActionTriggering.Start){
				DoAction( currentGesture);
			}
			
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_LongTap2Fingers && actionTriggering == ActionTriggering.InProgress){
				DoAction( currentGesture);
			}
			
			if (currentGesture.type == Engine.EasyTouch.EvtType.On_LongTapEnd2Fingers && actionTriggering == ActionTriggering.End){
				DoAction( currentGesture);
			}
		}
	}
