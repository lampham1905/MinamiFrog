...
{
	[CustomEditor(typeof(Engine.EasyTouch))]
	public class EasyTouchInspector : UnityEditor.Editor {

	
		public override void OnInspectorGUI(){
			
			Engine.EasyTouch t = (Engine.EasyTouch)target;
			...
			if (t.showGestureInspector){
				HTGuiTools.BeginGroup();{
					t.gesturePriority =(Engine.EasyTouch.GesturePriority) EditorGUILayout.EnumPopup("Priority to",t.gesturePriority );
					...
					t.doubleTapDetection = (Engine.EasyTouch.DoubleTapDetection) EditorGUILayout.EnumPopup("Double tap detection",t.doubleTapDetection);
					if (t.doubleTapDetection == Engine.EasyTouch.DoubleTapDetection.ByTime){
			...
			if (t.showTwoFingerInspector){
				HTGuiTools.BeginGroup();{
					...
					if (t.enable2FingersGesture){
						...
						t.twoFingerPickMethod = (Engine.EasyTouch.TwoFingerPickMethod)EditorGUILayout.EnumPopup("Pick method",t.twoFingerPickMethod);
