...
public class QuickBase : MonoBehaviour {
	...
	public virtual void Start(){
		...
		case GameObjectType.Obj_3D:
			LayerMask mask = Engine.EasyTouch.Get3DPickableLayer();
			mask = mask | 1<<gameObject.layer;
			Engine.EasyTouch.Set3DPickableLayer( mask);
			break;
			//2D
		case GameObjectType.Obj_2D:
			Engine.EasyTouch.SetEnable2DCollider( true);
			mask = Engine.EasyTouch.Get2DPickableLayer();
			mask = mask | 1<<gameObject.layer;
			Engine.EasyTouch.Set2DPickableLayer( mask);
			break;
			// UI
		case GameObjectType.UI:
			Engine.EasyTouch.instance.enableUIMode = true;
			Engine.EasyTouch.SetUICompatibily( false);
			break;
		}
